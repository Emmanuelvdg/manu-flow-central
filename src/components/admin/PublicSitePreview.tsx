...
export const PublicSitePreview: React.FC = () => {
  ...
  return (
    <div className="w-full bg-gray-50 border rounded-md overflow-hidden" style={{ backgroundColor: colorScheme.background }}>
      {/* Banner */}
      {banner.enabled && (
        <div 
      ...
      <main className="p-6">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold" style={{ color: colorScheme.text }}>Our Products</h1>
          <p className="mt-4 text-lg" style={{ color: `${colorScheme.text}99` }}>
        ...
        <div className={config.layout === 'grid' ? 'grid grid-cols-3 gap-4' : 'space-y-4'}>
          {[1, 2, 3].map(i => (
            <div 
              ...
              style={{ borderColor: `${colorScheme.text}20`, backgroundColor: `${colorScheme.background === '#ffffff' ? '#f9fafb' : colorScheme.background}` }}
            >
              ...
              <div className={config.layout === 'list' ? 'flex-1' : 'mt-4'}>
                <h3 className="font-medium" style={{ color: colorScheme.text }}>Product {i}</h3>
                <p className="text-sm mt-1" style={{ color: `${colorScheme.text}80` }}>Product description here</p>
      ...
      {/* Footer */}
      <footer className="border-t p-6" style={{ borderColor: `${colorScheme.text}20`, backgroundColor: headerStyle.backgroundColor }}>
        <div className="grid grid-cols-3 gap-4">
          {/* Company Info */}
          <div>
            <h3 className="text-lg font-semibold mb-2" style={{ color: colorScheme.text }}>{companyName}</h3>
            <p className="text-sm" style={{ color: `${colorScheme.text}99` }}>{contactInfo.address}</p>
          </div>

          {/* Contact Info */}
          <div>
            <h3 className="text-lg font-semibold mb-2" style={{ color: colorScheme.text }}>Contact Us</h3>
            <p className="text-sm" style={{ color: `${colorScheme.text}99` }}>Email: {contactInfo.email}</p>
            <p className="text-sm" style={{ color: `${colorScheme.text}99` }}>Phone: {contactInfo.phone}</p>
          </div>

          {/* Social Media */}
          <div>
            <h3 className="text-lg font-semibold mb-2" style={{ color: colorScheme.text }}>Follow Us</h3>
