...
import { ChevronDown, ChevronUp, Clock, Users, Factory, RefreshCcw } from "lucide-react";
...
export const OrderRoutingStages: React.FC<OrderRoutingStagesProps> = ({
  ...
  // Enhanced useEffect for debugging and fetching
  useEffect(() => {
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-4">
            <p className="text-red-500 mb-2">{loadingErrorMessage}</p>
            <Button onClick={handleRefresh} variant="outline">
              <RefreshCcw className="h-4 w-4 mr-2" />
              Try Again
            </Button>
          </div>
        ) : Object.keys(routingStages).length === 0 ? (
